...
class VaultStatsModel60D {
  VaultStatsModel60D({
    ...
    this.vaultStatsModelGraph60D,
    this.collectible,
    this.comic,
  });

  VaultStatsModel60D.fromJson(dynamic json) {
    ...
    if (json['graph'] != null) {
      vaultStatsModelGraph60D = [];
      json['graph'].forEach((v) {
        vaultStatsModelGraph60D?.add(VaultStatsModelGraph60D.fromJson(v));
      });
    }
  }
  ...
  List<VaultStatsModelGraph60D>? vaultStatsModelGraph60D;
  ...
  Map<String, dynamic> toJson() {
    ...
    if (vaultStatsModelGraph60D != null) {
      map['graph'] = vaultStatsModelGraph60D?.map((v) => v.toJson()).toList();
    }
    return map;
  }
}

class VaultStatsModelGraph60D {
  VaultStatsModelGraph60D({
    this.hour,
    this.total,
    this.inHour,
  ...
  VaultStatsModelGraph60D.fromJson(dynamic json) {
    hour = json['hour'];
    total = json['total'];
