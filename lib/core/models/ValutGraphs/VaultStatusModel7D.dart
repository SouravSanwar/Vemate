...
class VaultStatsModel7D {
  VaultStatsModel7D({
    ...
    this.vaultStatsModelGraph7D,
    this.collectible,
    this.comic,
  });

  VaultStatsModel7D.fromJson(dynamic json) {
    ...
    if (json['graph'] != null) {
      vaultStatsModelGraph7D = [];
      json['graph'].forEach((v) {
        vaultStatsModelGraph7D?.add(VaultStatsModelGraph7D.fromJson(v));
      });
    }
  }
  ...
  List<VaultStatsModelGraph7D>? vaultStatsModelGraph7D;
  ...
  Map<String, dynamic> toJson() {
    ...
    if (vaultStatsModelGraph7D != null) {
      map['graph'] = vaultStatsModelGraph7D?.map((v) => v.toJson()).toList();
    }
    return map;
  }
}

class VaultStatsModelGraph7D {
  VaultStatsModelGraph7D({
    this.hour,
    this.total,
    this.inHour,
  ...
  VaultStatsModelGraph7D.fromJson(dynamic json) {
    hour = json['hour'];
    total = json['total'];
