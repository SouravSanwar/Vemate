...
class SevenDayProductGraphPage extends StatefulWidget {
  final bool? fromVault;
  final String? ap;

  const SevenDayProductGraphPage({Key? key, this.fromVault, this.ap}) : super(key: key);

  @override
  State<SevenDayProductGraphPage> createState() => _SevenDayProductGraphPageState();
}

class _SevenDayProductGraphPageState extends State<SevenDayProductGraphPage> {
          ...
          child: data.sevenDayGraphModel != null
            ...
            axes: [
              NumericAxis(
                  ...
                  double.parse(detailsAp!) > maxPriceSevenDayGraph!
                      ? double.parse(detailsAp!) + (double.parse(detailsAp!)*.1)
                      : maxPriceSevenDayGraph!+(maxPriceSevenDayGraph!*.2)
                      :null,
            ...
            primaryYAxis: NumericAxis(
                ...
                double.parse(detailsAp!) > maxPriceSevenDayGraph!
                    ? double.parse(detailsAp!) + (double.parse(detailsAp!)*.1)
                    : maxPriceSevenDayGraph!+(maxPriceSevenDayGraph!*.1)
                    :null,
                minimum: widget.fromVault == true ? 0 :null
            ),
            series: <ChartSeries<SevenDayProductGraph, String>>[
              data.sevenDayGraphModel!.graphData!.graph!.length == 1
                ...
                xValueMapper: (plot, _) => data.sevenDayGraphModel!.graphData!.status == 0
                    ? plot.dayWiseTime
                    : plot.dayWiseTime1,
